...
object start {
  ...
  def excesecond(): Unit = {
    //导出数据文件到指定目录
    val filename = (configMess.getTimestamp() + "_file.txt").substring(1, (configMess.getTimestamp() + "_file.txt").size)
    ...
    taskMessLog.setFileSize(new File(configMess.getFTPFileSourceLocation() + filename).getUsableSpace)
